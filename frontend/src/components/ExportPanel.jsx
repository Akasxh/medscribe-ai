import { Download, FileText, Copy, Check, Printer, Share2 } from 'lucide-react'
...
export default function ExportPanel({ clinicalNote, fhirBundle }) {
  const handleWhatsAppShare = useCallback(() => {
    ...
    const lines = ['*MedScribe AI - Prescription*']
    ...
    if (medications?.length) {
      ...
      medications.forEach(m => {
        lines.push(`- ${m.name}${m.generic_name ? ` (${m.generic_name})` : ''} ${m.dosage || ''} ${m.frequency || ''} ${m.duration ? `for ${m.duration}` : ''}`.trim())
      })
    }
    if (follow_up) lines.push(`*F/U:* ${follow_up}`)
    lines.push('\n_Generated by MedScribe AI_')
